...
3. **점프 높이 240px 고정** (V2에서 300→240, 사용자 난이도 피드백): 난이도는 바운스 주기(0.90초→0.40초)로만 상승. 천장 가시는 y=318 기준(점프 정점에서만 닿음 — 수평 타이밍 회피). 정본은 `utils/constants.ts`
...
### V2 기믹 규칙 (2026-06 기믹 확장 — PLAN_V2_기믹확장.md가 설계 정본)

- **점멸 = 상태 문법 통일**: 실선 채움=실체 / 빠른 점멸=곧 소멸 / 점선 윤곽=곧 등장·허상 경고 / 윤곽선만=허상. 새 요소도 이 문법을 따를 것
- **공정성 3원칙** (모든 신규 위험물): ① 스폰 후 3초 무입력 생존 ② 위험은 반드시 점멸/점선 예고 ③ 사망 판정은 시각 크기의 75~85%로 후하게
- **launcher는 다른 발판과 수평으로 겹치지 말 것** — 겹치면 넓은 발판이 착지 판정을 가져가 발사가 안 됨 (Collision bestFloor 선택 규칙)
- **점멸·이동 가시의 시계는 stageMs**(timeMs−introStart) — 충돌(Collision)과 렌더(Renderer)가 `game/physics/blink.ts`의 같은 함수를 쓴다. 한쪽만 바꾸면 보이는 것과 죽는 것이 어긋난다
- **chase(추격전) 스테이지엔 구덩이 금지** — 추격 중 낙사 회복 불가. 가시·붕괴 발판·반동 부스터만
- **금 간 벽에는 반드시 반경 170 안에 폭탄** — 없으면 소프트락 (validate2가 검사함)
...
# 스테이지 데이터·물리를 바꿨으면 (프로젝트 루트에서):
npx esbuild app/src/game/stages/stages.ts --bundle --format=cjs --outfile=.tmp/stages.cjs
node .tmp/validate2.cjs     # 구멍 도달성·스폰 안전·천장가시·금간벽-폭탄 짝·이동가시
node .tmp/climb-sim.cjs     # 구덩이 지그재그 탈출·샤프트 등반·S13 (벽타기 전수)
node .tmp/gimmick-sim.cjs   # 넉백 제동·등지기 생존·S12 하강 전선·S19 추격 여유·점멸 창
node .tmp/verify-browser.cjs # headless 실플레이 (Stage1 클리어 + 콘솔 오류 0)
node .tmp/camera-sim.cjs    # 카메라 데드존 (멀미 방지 7항목)
```

- 수정 후 최소 tsc + build를 돌리고 결과를 보고할 것
- 물리·충돌·스테이지 데이터를 바꿨으면 위 시뮬레이터 전부 + 브라우저 실제 플레이로 확인할 것
