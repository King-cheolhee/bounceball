 ...
 * - launcher: 다른 발판과 수평으로 겹치지 말 것 (겹치면 넓은 발판이 착지 판정을 가져감)
...
const STAGES: StageData[] = [
  ...
  stage(13, '자유 낙하', 900, { x: 590, y: 360, width: 110, height: 110 }, [
    ...
    floorAt(240, 1200, 240, 'fragile'), // 샤프트 바닥 — 1회용. 실패 = 추락
    floorAt(660, 1280, 240, 'fragile'), // 탈출구 아래 회수용 바닥 — 역시 1회용 (소프트락 방지)
    part(350, 1000), part(360, 760), part(330, 520), part(560, 340),
  ], { height: 1320, spawn: { x: 300, y: 80 }, hint: '발판은 한 번뿐 — 벽을 타라' }),
  ...
  stage(20, '코어 점화', 2620, { x: 2420, y: 100, width: 110, height: 110 }, [
    ...
    // V2: 점멸 벽 — 주기적으로 우측 벽면이 돌출. 켜지면 샤프트가 좁아지고(가운데 통로는
    // 항상 열려 있음), 벽 반동의 추가 발판이 된다 — 점멸 박자 + 벽 반동의 최종 시험
    { type: 'wall', x: 2560, y: 480, width: 60, height: 160, blinkPeriodMult: 3 },
